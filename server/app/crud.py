...
async def search_places(db: AsyncSession, unified_search_term: str="", place_name: str = "", road_address_name: str = "", place_description: str = ""):
    ...
    if unified_search_term: # 여러 필드에서 합집합으로 부분일치 검색
        unified_filters = or_(
            models.Place.place_name.ilike(f"%{unified_search_term}%"),
            models.Place.road_address_name.ilike(f"%{unified_search_term}%"),
            models.Place.place_description.ilike(f"%{unified_search_term}%")
        )
        filters.append(unified_filters)

    if place_name:
        filters.append(models.Place.place_name.ilike(f"%{place_name}%"))  # 부분 일치 검색 (대소문자 구분 없음)
    
    if road_address_name:
        filters.append(models.Place.road_address_name.ilike(f"%{road_address_name}%"))
    
    if place_description:
        filters.append(models.Place.place_description.ilike(f"%{place_description}%"))

    if filters:
        query = select(models.Place).where(and_(*filters))  # *filters는 리스트의 모든 항목을 조건으로 넣음
    else:
        query = select(models.Place)  # 검색어가 아무것도 없을 경우 전체 조회
